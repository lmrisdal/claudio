...
public class AuthConfig
{
    ...
    public OidcProviderConfig FirstOidcProvider()
    {
        if (OidcProviders.Count == 0)
            OidcProviders.Add(new OidcProviderConfig());

        return OidcProviders[0];
    }

    public IEnumerable<OidcProviderConfig> ConfiguredOidcProviders() =>
        OidcProviders.Where(provider => provider.IsConfigured);

    public OidcProviderConfig? FindOidcProvider(string slug) =>
        ConfiguredOidcProviders().FirstOrDefault(provider =>
...
public class LibraryConfig
{
    ...
    public int ScanIntervalSecs { get; set; } = 120;
}
