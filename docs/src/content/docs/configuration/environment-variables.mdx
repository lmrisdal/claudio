...
`CLAUDIO_BROWSER_ORIGINS` is only needed when the browser app and API are served from different origins. For multiple frontend origins, use a comma-separated list with no spaces.
