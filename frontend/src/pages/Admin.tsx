...
function UsersTab() {
  ...
  const addUserMutation = useMutation({
    mutationFn: () => api.post('/auth/register', { username: newUsername, password: newPassword }),
    onSuccess: () => {
      ...
      {showAddUser && (
        <div className="card bg-surface rounded-xl p-5 ring-1 ring-border">
          ...
          >
            ...
            <button
              type="button"
              onClick={() => { setShowAddUser(false); setNewUsername(''); setNewPassword(''); setAddError('') }}
      ...
      <div className="card bg-surface rounded-xl ring-1 ring-border overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={4} className="px-5 py-8 text-center text-text-muted">Loading...</td>
              </tr>
            ) : users.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-5 py-8 text-center text-text-muted">No users</td>
              </tr>
            ) : (
              users.map((user) => (
                <tr key={user.id} className="border-b border-border/50 hover:bg-surface-raised/50 transition-colors">
...
function GamesTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => { setDeleteTarget(null); setDeleteFiles(false) }}>
          <div className="bg-surface rounded-xl ring-1 ring-border p-6 max-w-sm w-full mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-text-primary font-medium mb-2">Delete game</h3>
            <p className="text-sm text-text-secondary mb-4">
              Remove <span className="font-medium text-text-primary">"{deleteTarget.title}"</span> from the database?
            </p>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-text-secondary text-sm mb-4">
              Walk the game library directories for new or changed games. Existing games will have their file sizes updated.
            </p>
            <button
              onClick={triggerScan}
              disabled={scanning}
        ...
        {result && (
          <div className="mt-4 ml-14 bg-accent-dim rounded-lg px-4 py-3">
            <p className="text-sm text-accent font-medium">
              Scan complete — {result.gamesFound} found, {result.gamesAdded} added, {result.gamesMissing} missing
            </p>
          </div>
        )}
      ...
      <div className="card bg-surface rounded-xl p-6 ring-1 ring-border">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-text-secondary text-sm mb-4">
              Fetch metadata from IGDB for games that haven't been matched yet — covers, summaries, genres, and release years.
            </p>
            <button
