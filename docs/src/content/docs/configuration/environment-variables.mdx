...
`CLAUDIO_BROWSER_ORIGINS` is only needed when the browser app and API are served from different origins. Most deployments should leave it unset. For multiple frontend origins, use a comma-separated list with no spaces.
